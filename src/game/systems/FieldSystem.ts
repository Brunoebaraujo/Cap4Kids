...
export interface PlotRecord { id: string; fieldId: number; row: number; col: number; state: FieldState }
...
function createPlots(fieldId: number, unlocked: boolean): PlotRecord[] {
  ...
  for (let row = 0; row < GRID_SIZE; row += 1) {
    for (let col = 0; col < GRID_SIZE; col += 1) {
      plots.push({ id: 'field-' + fieldId + '-plot-' + row + '-' + col, fieldId, row, col, state: unlocked ? 'Raw' : 'Locked' });
    }
  }
  return plots;
}
...
export class FieldSystem {
  ...
  plant(plotIdOrTileX: string | number, tileY?: number) { const plot = typeof plotIdOrTileX === 'string' ? this.getPlotById(plotIdOrTileX) : this.getFirstPlotWithState('Prepared'); if (!plot || plot.state !== 'Prepared') return false; plot.state = 'Planted'; return true; }
  harvest(plotIdOrTileX: string | number, tileY?: number) { const plot = typeof plotIdOrTileX === 'string' ? this.getPlotById(plotIdOrTileX) : this.getFirstPlotWithState('Mature'); if (!plot || plot.state !== 'Mature') return false; plot.state = 'Raw'; return true; }

  advanceDay() { let changed = false; this.allPlots.forEach((plot) => { if (plot.state === 'Planted') { plot.state = 'Growing'; changed = true; } else if (plot.state === 'Growing') { plot.state = 'Mature'; changed = true; } }); return changed; }
